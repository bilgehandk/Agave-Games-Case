...
{
    public static List<Vector3> GenerateSmoothCurve(List<Vector3> points, int segmentsPerCurve = 10)
    {
        if (points.Count < 2)
            return points;
        ...
        for (int i = 0; i < points.Count - 1; i++)
        {
            ...
            if (i > 0 && i < points.Count - 2)
            {
                ...
                control0 = p0 + tangent0 * (distance * 0.3f);
                control1 = p1 - tangent1 * (distance * 0.3f);
            }
            else
            {
                control0 = p0 + (p1 - p0) * 0.3f;
                control1 = p1 - (p1 - p0) * 0.3f;
            }
    ...
    public static void ApplySmoothCurve(LineRenderer lineRenderer, List<Vector3> points, int segmentsPerCurve = 10)
    {
